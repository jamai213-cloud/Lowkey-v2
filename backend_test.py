...
# Configuration
BASE_URL = "https://social-connect-592.preview.emergentagent.com/api"
    ...
    def test_auth_reset_password(self):
        ...
        if response:
            if response.status_code == 400:
                ...
                self.log(f"❌ Reset password unexpected status code: {response.status_code}", "ERROR")
        else:
            self.log("❌ Reset password request failed - no response", "ERROR")
        return False
